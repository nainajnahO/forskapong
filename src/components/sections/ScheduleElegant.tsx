...
export default function ScheduleElegant({ id }: ScheduleElegantProps) {
  ...
  return (
    <section id={id} className="w-full py-16 md:py-24 bg-black relative overflow-hidden">
      {/* Subtle background pattern */}
      <div className="absolute inset-0 opacity-5" style={{
        backgroundImage: 'radial-gradient(circle at 2px 2px, rgb(113 113 122) 1px, transparent 0)',
      ...
      <Container>
        ...
        <div className="mb-20 relative z-10">
          ...
          <div className="flex items-center gap-2 mt-6">
            <div className="h-px w-16 bg-gradient-to-r from-transparent via-zinc-700 to-transparent"></div>
        ...
        <div ref={timelineRef} className="relative max-w-4xl mx-auto">
          ...
          <div className="absolute left-8 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-red-500/30 via-zinc-700/50 to-red-500/30 hidden md:block"></div>
                ...
                <div className={`relative ${
                  ...
                  <div>
                    ...
                    <div className="text-center mb-8">
                      <h3 className="text-2xl md:text-3xl text-white font-display font-bold inline-block">
                        {phase.name}
                    ...
                    <div className="space-y-6">
                      {phase.events.map((event: ScheduleEvent, eventIndex) => (
                        <div
                          key={eventIndex}
                          className="group bg-zinc-900/40 backdrop-blur-sm border border-zinc-800/50 rounded-lg p-6 hover:bg-zinc-900/60 hover:border-red-900/50 transition-all duration-500 shadow-lg hover:shadow-red-900/20"
                        >
                          {/* Time */}
                          ...
                          {/* Event title */}
                          <h4 className={`text-xl md:text-2xl mb-2 text-white font-display ${
                          ...
                          {event.speakers && event.speakers.length > 0 && (
                            <div className="mt-5 pt-5 border-t border-zinc-800/30">
                              <div className="flex flex-wrap gap-4">
                                {event.speakers.map((speaker, speakerIndex) => (
                                  <div
                                    key={speakerIndex}
                                    className="flex items-center gap-3 bg-zinc-800/30 rounded-lg px-4 py-3 border border-zinc-700/30"
                                  >
                                    <div className="w-11 h-11 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-800 flex items-center justify-center flex-shrink-0">
                                      <span className="text-xs text-zinc-500">
                                        {speaker.name.substring(0, 2).toUpperCase()}
                                      </span>
                                    </div>
                                    <div>
                                      <p className="text-sm text-white font-semibold">{speaker.name}</p>
